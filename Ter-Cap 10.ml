...
module Term = 
	...
	let rec typeof ctx t =
		...
		| TmPred(t) ->  if (=) (typeof ctx t) TyNat then TyNat
									else raise (Err "t hasn't a predecessor")
		
		| TmSucc(t) -> if (=) (typeof ctx t) TyNat then TyNat
									else raise (Err "t hasn't a successor")
									
		| TmIsZero(t) -> if (=) (typeof ctx t) TyNat then TyBool
									else raise (Err "isZero must be applied to Nat")
		
		| TmIf(t1,t2,t3) ->  if (=) (typeof ctx t1) TyBool then
									let tyT2 = typeof ctx t2 in
									if (=) tyT2 (typeof ctx t3) then tyT2
									else raise (Err "arms of conditional have different types")
								else raise (Err "guard of conditional not a boolean")
		...
		| TmApp(t1,t2) -> 	let tyT1 = typeof ctx t1 in
								...
								(match tyT1 with
									| TyArr(tyT11,tyT12) -> if (=) tyT2 tyT11 then tyT12
															else raise (Err "parameter type mismatch")
									| _ -> raise (Err "arrow type expected")
								)
	...
	let rec eval1 ctx t = 
		...
		| TmApp(TmAbs(x, ty, t12), v2) when isval v2 -> termSubstTop v2 t12             			(*E_APPABS*)
	...
	let rec printEvalStep ctx t = 
		...
		| TmApp(TmAbs(x, ty, t12), v2) when isval v2 -> Format.printf "E_APPABS \n"; termSubstTop v2 t12            			 (*E_APPABS*)
	...
	let rec printedBigstep ctx t =
		...
		(* Church booleans *)
		let tru = TmAbs("t", TyBool, TmAbs("f", TyBool, TmVar(1, 2)))
		let fls = TmAbs("t", TyBool, TmAbs("f", TyBool, TmVar(0, 2)))

		let orr = TmAbs("b", TyBool, TmAbs("c", TyBool, TmApp(TmApp(TmVar(1, 2), (termShift 2 tru)), TmVar(0, 2))))
		let andd = TmAbs("b", TyBool, TmAbs("c", TyBool, TmApp(TmApp(TmVar(1, 2), TmVar(0, 2)), (termShift 2 fls))))
		let nott = TmAbs("a", TyBool, TmApp(TmApp(TmVar(0, 1), (termShift 1 fls)), (termShift 1 tru)))


		let c0 = TmAbs("s", TyNat, TmAbs("z", TyNat, TmVar(0, 2)))
		let c1 = TmAbs("s", TyNat, TmAbs("z", TyNat, TmApp(TmVar(1, 2), TmVar(0, 2))))
